...
void loop() {
  if (radio.available()) {
    ...
    switch (commandID) {
      ...
      case 2: turnY(); break;
      case 3: turnXY(); break;
      case 4: turn_negXY(); break; // Исправлено имя функции
    }
    Serial.println("Последовательность завершена. Возврат в режим ожидания.");
  }
}

// --- Ваши последовательности, дополненные отправкой отчетов ---

void turnX() {
  digitalWrite(Lazer_Pin, HIGH);
  for(int i = 90; i <= 130; i += 10) { servo1.write(i); delay(500); sendFeedback(); }
  for(int i = 120; i >= 50; i -= 10) { servo1.write(i); delay(500); sendFeedback(); }
  for(int i = 60; i <= 90; i += 10) { servo1.write(i); delay(500); sendFeedback(); }
  digitalWrite(Lazer_Pin, LOW);
}

void turnY() {
  digitalWrite(Lazer_Pin, HIGH);
  for(int i = 90; i <= 130; i += 10) { servo2.write(i); delay(500); sendFeedback(); }
  for(int i = 120; i >= 50; i -= 10) { servo2.write(i); delay(500); sendFeedback(); }
  for(int i = 60; i <= 90; i += 10) { servo2.write(i); delay(500); sendFeedback(); }
  digitalWrite(Lazer_Pin, LOW);
}

void turnXY() {
  digitalWrite(Lazer_Pin, HIGH);
  for(int i = 90; i <= 130; i += 10) { servo1.write(i); servo2.write(i); delay(500); sendFeedback(); }
  for(int i = 120; i >= 50; i -= 10) { servo1.write(i); servo2.write(i); delay(500); sendFeedback(); }
  for(int i = 60; i <= 90; i += 10) { servo1.write(i); servo2.write(i); delay(500); sendFeedback(); }
  digitalWrite(Lazer_Pin, LOW);
}
...
void turn_negXY() {
  digitalWrite(Lazer_Pin, HIGH);
  for(int i = 0; i <= 4; i++) { 
    servo1.write(90 - i*10); 
    servo2.write(90 + i*10); 
    delay(500); 
    sendFeedback(); 
  }
  for(int i = 3; i >= 0; i--) { 
